...
namespace mame
{
    public class FM
    {
        ...
        public static void ym2610b_update_one(int offset, int length)
        {
            int i, j;
            /* refresh PG and EG */
            refresh_fc_eg_chan(0);
            refresh_fc_eg_chan(1);
            if ((ST.mode & 0xc0) != 0)
            {
                /* 3SLOT MODE */
            ...
            for (i = 0; i < length; i++)
            {
                ...
                while (OPN.eg_timer >= OPN.eg_timer_overflow)
                {
                    OPN.eg_timer -= OPN.eg_timer_overflow;
                    OPN.eg_cnt++;
                    advance_eg_channel(0);
                    advance_eg_channel(1);
                    advance_eg_channel(2);
                    advance_eg_channel(3);
                    advance_eg_channel(4);
                    advance_eg_channel(5);
                }
                ...
                lt += (out_delta[2] + out_delta[3]) >> 9;
                rt += (out_delta[1] + out_delta[3]) >> 9;
                lt += (int)((out_fm[0] >> 1) & OPN.pan[0]);	/* the shift right is verified on YM2610 */
                rt += (int)((out_fm[0] >> 1) & OPN.pan[1]);
                lt += (int)((out_fm[1] >> 1) & OPN.pan[2]);
                rt += (int)((out_fm[1] >> 1) & OPN.pan[3]);
                lt += (int)((out_fm[2] >> 1) & OPN.pan[4]);
                rt += (int)((out_fm[2] >> 1) & OPN.pan[5]);
                lt += (int)((out_fm[3] >> 1) & OPN.pan[6]);
                rt += (int)((out_fm[3] >> 1) & OPN.pan[7]);
                lt += (int)((out_fm[4] >> 1) & OPN.pan[8]);
                rt += (int)((out_fm[4] >> 1) & OPN.pan[9]);
                lt += (int)((out_fm[5] >> 1) & OPN.pan[10]);
                rt += (int)((out_fm[5] >> 1) & OPN.pan[11]);
