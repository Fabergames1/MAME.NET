...
namespace mame
{
    public class K054539
    {
        ...
        public static void k054539_init_chip(/*const char *tag, struct k054539_info *info,*/ int clock)
        {
            int i;
            ...
            else
            {
                switch (offset)
                {
                    ...
                    case 0x22d:
                        ...
                        break;
                    case 0x22e:
                        //info.cur_zone = (data == 0x80 ? info.ram : k054539rom + 0x20000 * data);
                        if(data==0x80)
                        {
                            zoneflag = 1;
                        }
            ...
            switch (offset)
            {
                case 0x22d:
                    if ((info.regs[0x22f] & 0x10) != 0)
                    {
                        ...
                        //res= info.cur_zone[info.cur_ptr];
                        info.cur_ptr++;
                        if (info.cur_ptr == info.cur_limit)
                        {
                            info.cur_ptr = 0;
